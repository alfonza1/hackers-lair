...
async function repositoryRoot(directory) {
  try {
    return path.resolve(await git(directory, ['rev-parse', '--show-toplevel']));
  } catch {
    return '';
  }
}
...
async function lastTouchedForSkills(skills, {
  ...
  const roots = await Promise.all(candidates.map(async (skill) => ({
    ...skill,
    repositoryRoot: await repositoryRoot(skill.directory),
  })));
  const groups = new Map();
  for (const skill of roots.filter((item) => item.repositoryRoot)) {
    const relativeDirectory = normalizeGitPath(path.relative(skill.repositoryRoot, skill.directory));
    const group = groups.get(skill.repositoryRoot) || [];
    group.push({ id: skill.id, relativeDirectory });
    groups.set(skill.repositoryRoot, group);
  }
