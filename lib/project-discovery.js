...
const localWrapper = (filename) => IS_WINDOWS ? filename : `./${filename}`;
    ...
    return {
      ...
      components: [{
        ...
        command: fs.existsSync(mavenWrapper)
          ? `${localWrapper(path.basename(mavenWrapper))} spring-boot:run`
          : 'mvn spring-boot:run',
        match: directory,
      }],
    };
    ...
    return {
      ...
      components: [{
        ...
        command: fs.existsSync(gradleWrapper)
          ? `${localWrapper(path.basename(gradleWrapper))} bootRun`
          : 'gradle bootRun',
        match: directory,
      }],
    };
