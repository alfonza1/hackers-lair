...
function repositoryStatus(directory) {
  ...
  try {
    ...
    let commitCount = 0;
    try {
      commitCount = Number(runGit(root, ['rev-list', '--count', 'HEAD'])) || 0;
    } catch {
      // An initialized repository without a first commit has a local count of zero.
    }
    const value = { ...parsePorcelainStatus(output, root), commitCount };
