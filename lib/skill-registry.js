...
function fileRecord(skillFile, { llm, kind, origin, namespace = '' }) {
  ...
  return {
    id: `${llm.toLowerCase()}-${kind}-${name.toLowerCase()}`,
    name,
    description: parsed.description,
    llm,
    kind,
    origin,
    invocation: llm === 'Claude' ? `/${name}` : `$${name}`,
...
function latestUniqueRecords(records) {
  ...
  for (const record of records) {
    const key = `${record.llm}:${record.kind}:${record.name}`.toLowerCase();
  ...
  return [...unique.values()]
    ...
    .sort((left, right) => (
      ...
      || left.llm.localeCompare(right.llm)
...
function listSkills(options = {}) {
  ...
  const records = [
    ...recordsFromFiles(personalSkillFiles(codexSkills), () => ({ llm: 'Codex', kind: 'personal', origin: 'Personal' })),
    ...recordsFromFiles(personalSkillFiles(claudeSkills), () => ({ llm: 'Claude', kind: 'personal', origin: 'Personal' })),
    ...recordsFromFiles(nestedSkillFiles(path.join(codexSkills, '.system')), () => ({ llm: 'Codex', kind: 'default', origin: 'System' })),
