...
function canonicalDirectoryKey(directory) {
  const canonical = canonicalDirectory(directory);
  return process.platform === 'win32' ? canonical.toLowerCase() : canonical;
}

function mergePhysicalUserRecords(records) {
  const unique = new Map();
  for (const record of records) {
    const key = canonicalDirectoryKey(record.directory);
    const existing = unique.get(key);
    if (!existing) {
      unique.set(key, record);
      continue;
    }
    const preferred = existing.kind === 'personal'
      ? existing
      : record.kind === 'personal'
        ? record
        : record.modifiedAt > existing.modifiedAt ? record : existing;
    unique.set(key, {
      ...preferred,
      harnesses: [...new Set([
        ...(existing.harnesses || []),
...
function userSkillRecords({
  ...
  return mergePhysicalUserRecords(records);
}
