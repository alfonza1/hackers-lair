...
function localInferenceSetupPrompt(snapshot) {
  ...
  if (!snapshot.supported) {
    return [
      `Hacker's Lair local inference controls require native Windows.`,
  ...
  const launcherInventory = snapshot.models
    ...
    .join('\n');
  return [
    `Set up Hacker's Lair local inference on this native Windows machine.`,
    '',
    ...
    `7. Finish in Hacker's Lair > Local Inference: confirm neither channel shows MISSING, then start each model separately and confirm it reaches ONLINE with no conflict. Report every file changed and both verification results.`,
    '',
