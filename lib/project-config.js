...
const KNOWN_PROJECT_FIELDS = new Set(['name', 'type', 'components']);
...
function preserveUnknownProjectFields(existing, normalized) {
  ...
  const components = normalized.components.map((component, index) => {
    const prior = existingComponents.find((candidate) => (
      String(candidate.name).toLowerCase() === component.name.toLowerCase()
    )) || existingComponents[index];
    return {
      ...unknownFields(prior, KNOWN_COMPONENT_FIELDS),
      ...component,
    };
  });
  return {
    ...unknownFields(existing, KNOWN_PROJECT_FIELDS),
    ...normalized,
    components,
  };
}
...
function updateProjectConfig(config, { originalName = '', project }, options = {}) {
  ...
  if (editingIndex >= 0) {
    nextProjects[editingIndex] = preserveUnknownProjectFields(
      currentProjects[editingIndex],
      normalized,
    );
  } else {
