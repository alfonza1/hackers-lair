...
function createWin32Platform({ runCommand = run } = {}) {
  return {
    name: 'win32',
    ...
    async openTarget(action, target) {
      if (action === 'logs') return spawnDetached('notepad.exe', [target.logFile]);
      if (action === 'explorer') return spawnDetached('explorer.exe', [target.cwd]);
      if (action === 'terminal') return spawnDetached('cmd.exe', ['/d', '/k'], { cwd: target.cwd });
      if (action === 'vscode') return spawnDetached('code.cmd', [target.cwd], { cwd: target.cwd });
